...
conn = pymysql.connect(host='localhost', user='root', password='theanimesh2005', db='school_db', autocommit=True)

with conn.cursor() as cursor:
    cursor.execute('DELETE FROM users WHERE id IN (10001, 10002, 10003, 10004, 10005, 10006)')
    cursor.execute('DELETE FROM student_page1 WHERE user_id IN (10001, 10002, 10003, 10004, 10005, 10006)')
    cursor.execute('DELETE FROM student_page2 WHERE user_id IN (10001, 10002, 10003, 10004, 10005, 10006)')
    cursor.execute('DELETE FROM student_page3 WHERE user_id IN (10001, 10002, 10003, 10004, 10005, 10006)')
    cursor.execute('DELETE FROM student_page4 WHERE user_id IN (10001, 10002, 10003, 10004, 10005, 10006)')
    cursor.execute('DELETE FROM school_students WHERE id IN (10001, 10002, 10003, 10004, 10005, 10006)')
    ...
    for row in data:
        ...
        cursor.execute('INSERT INTO student_page1 (user_id, name, admission_number, class, roll_number) VALUES (%s, %s, %s, %s, %s)',
                       [user_id, name, adm_no, cls, roll])
                       
        cursor.execute('INSERT INTO student_page2 (user_id, dob, mother_tongue) VALUES (%s, %s, %s)',
                       [user_id, dob, lang])
                       
        cursor.execute('INSERT INTO student_page3 (user_id, address, contact) VALUES (%s, %s, %s)',
                       [user_id, address, contact])
                       
        cursor.execute('INSERT INTO student_page4 (user_id, father_name, mother_name, father_contact, mother_contact) VALUES (%s, %s, %s, %s, %s)',
                       [user_id, fname, mname, contact, mcontact])
                       
        cursor.execute('INSERT INTO school_students (id, name, roll_number) VALUES (%s, %s, %s)',
                       [user_id, name, roll])
