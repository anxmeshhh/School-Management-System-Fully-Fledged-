...
df = pd.read_excel('DATABASE 26-27.xlsx', sheet_name='KILPAUK DB 26-27')
...
for index, row in df.iterrows():
    ...
    if not pd.isna(dob):
        if isinstance(dob, pd.Timestamp):
            dob_sql = f"'{dob.strftime('%Y-%m-%d')}'"
        else:
            try:
                dt = pd.to_datetime(dob)
                dob_sql = f"'{dt.strftime('%Y-%m-%d')}'"
            except:
                dob_sql = f"'{dob}'"
        
    cls = row.iloc[5] # CLASS
    fname = row.iloc[6] # FATHER NAME
    contact = row.iloc[7] # CONTACT
    mname = row.iloc[8] # MOTHER NAME
    mcontact = row.iloc[9] # CONTAC T
    address = row.iloc[10] # ADDRESS
    ...
    print(f"-- {int(roll) if not pd.isna(roll) else index}. {name}")
    ...
    print(f"INSERT IGNORE INTO student_page2 (user_id, dob, mother_tongue, gender) VALUES ({user_id}, {dob_sql}, NULL, {esc(gender)});")
